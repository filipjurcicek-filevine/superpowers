...
Shared guidance for **Opus/Fable, Sol, and Astra-class models**. This package
still targets **Claude Code** — CLI and VS Code extension. Model-class scope does
not add another harness adapter or establish measured parity across models.
...
**Two existing effort defaults: judgment and execution.** These are Claude Code
defaults, not measured optima for all target models. Planning and every gate run
`medium`. Code implementation runs `low`. Preserve these defaults while prompts change.
Task briefs specify outcomes, constraints, and shared interfaces. Implementers
choose routine implementation details and reviewers check the resulting behavior.
Measure effort changes separately from prompt changes so their effects remain
attributable. Do not assume a capable model removes the need for verification.
...
instructions of the role. Reviewer agents omit edit tools but retain Bash for
inspection. Their no-mutation
contract is advisory unless runtime permissions enforce it. Do not describe this
tool list as a filesystem sandbox. Use runtime controls for hard boundaries.
...
against the rule "Say each rule once" below. Change wording for a clear purpose
and measure its effect. Earlier results
on one model do not establish that body length never affects another model.
...
failure mode. See `skills/writing-skills/SKILL.md`, section "Design the contract". When you add guidance, delete the text that it replaces.

**Do not repeat the system prompt.** Skills supply operational contracts and
non-obvious constraints. Remove generic coaching, repeated examples, and rules
already enforced by the runtime. Keep one shared prompt unless model-specific
evaluations demonstrate a need for a separate instruction.

**Scale process to the task.** Existing authorization persists. Ask only when a
missing decision changes scope, user outcomes, or required permission. Verify
review findings before fixing them, and reuse test evidence when its inputs stay
valid. Preserve explicit user verification gates and required project checks.
...
**Validation scope:** earlier Opus 5 changes lacked a verified behavioral baseline.
Evaluate Opus/Fable, Sol, and Astra separately using current and proposed prompts
plus a no-guidance control. Decision probes do not replace implementation-level
pressure tests. Record unavailable targets and failed harness runs as gaps, not
passes. Keep release claims within the evidence actually collected.
